...
template <class T>
static T cycle(T value) {
    auto ret = static_cast<int>(value) + 1;
    if (ret >= static_cast<int>(T::LAST_)) {
        ret = 0;
    }
...
public:
    ...
    static void setTool(SelectTool tool) {
        s_tool = tool;
    }

    static void setMode(SelectMode mode) {
        s_mode = mode;
    }
...
class $modify(SelectUI, EditorUI) {
    ...
    bool init(LevelEditorLayer* lel) {
        ...
        this->template addEventListener<InvokeBindFilter>([=](InvokeBindEvent* event) {
            if (event->isDown()) {
                if (!m_swipeEnabled) {
                    this->toggleSwipe(m_swipeBtn);
                }
                ASelect::setTool(cycle(ASelect::getTool()));
                if (m_fields->select) {
                    m_fields->select->updateUI();
                }
                if (m_fields->quickSelect) {
                    m_fields->quickSelect->updateUI();
                }
                this->updateSwipeImage();
            }
            return ListenerResult::Propagate;
        }, "next-swipe-tool"_spr);
        ...
        this->template addEventListener<InvokeBindFilter>([=](InvokeBindEvent* event) {
            if (event->isDown()) {
                ...
                ASelect::setMode(cycle(ASelect::getMode()));
                if (m_fields->select) {
                    m_fields->select->updateUI();
                }
                if (m_fields->quickSelect) {
...
$execute {
    BindManager::get()->registerBindable(BindableAction(
        "next-swipe-tool"_spr,
        "Switch Swipe Tool",
        "Switch between Swipe, Lasso, and Magic Wand selection. "
        "Also enables swipe, if it is not already enabled",
        {},
        Category::EDITOR_UI
